...
- Contact data lives in `content/settings.yml` → `build.py` writes it **directly into the HTML** (`.neuro-phone`, `.neuro-mail`, `.neuro-address`, tel/WhatsApp/mailto/Maps links, logo) in every place it's needed, and into `firma.html`. No JS is involved; `data.js` only carries modal content (`services`, `modalities` and `icons`). To change contact info, edit `settings.yml` (and, to avoid drift, the JSON-LD fields in the same file).
