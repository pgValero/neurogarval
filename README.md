...
2. **The build is the quality gate.** Every deploy is generated and linted, and the
   site is only published when the "Publish changes" button passes the build first,
   so a bad edit can never reach production.
...
| Safety | Fail-fast build, four linters, validation before every deploy, scheduled smoke test of the live site |
| Publishing | Editor saves → commit on `main` (triggers nothing) → clicks "Publish changes" → build + validation → automatic deploy |
...
flowchart TB
    ...
    subgraph L2["2 · REPOSITORY — Git is the database, the CMS and the audit trail"]
        MAIN["<b>branch main</b><br/>content/*.yml — the copy<br/>hero · services · modalities · process · blog · faq · contact"]
    ...
    subgraph L3["3 · AUTOMATION — free, hosted by GitHub, no server to maintain"]
        W3["deploy_page.yml<br/>triggered by the Publicar cambios button<br/>build job: build.py + prek linters<br/>deploy job: publishes _site/"]
    ...
    CMS -->|"1 · saves the form → YAML on main (triggers nothing)"| MAIN
    CMS -->|"1 · uploads photos"| ASSETS
    CMS -->|"2 · clicks Publicar cambios"| W3
...
| *What stops a typo from reaching production?* | `build.py` fails on any missing or undeclared field, the linters check the generated HTML/CSS/JS, and the deploy stops if the validation fails — the live site keeps the previous version. |
...
Technical work follows the exact same door: changes land on `main` and are
published with the same "Publicar cambios" button, so there is a single
publishing path for both the editor and a developer.
...
├── .github/workflows/  # deploy, live site check
...
sequenceDiagram
    ...
    participant C as Pages CMS · branch main
    participant G as GitHub repository
    participant W as GitHub Actions
    participant P as GitHub Pages

    E->>C: fills the form, writes Markdown, uploads a photo
    C->>G: commit to main — content/*.yml + media/ (triggers nothing)
    E->>C: clicks "Publicar cambios"
    C->>W: dispatch deploy_page.yml (the button is the trigger)
    W->>W: build job — build.py + prek on the generated _site/
    alt a check fails
        W-->>G: the deploy stops, the live site keeps the previous version
    else all checks pass
        W->>W: upload _site/ as the Pages artifact
...
uploads a file. Saving edits only commits to `main`; the only button that publishes
is **"Publicar cambios"**.

Technical work follows the same path: changes land on `main` and are published
with the same button. Content commits trigger nothing; only the deploy workflow
publishes.
...
8. **Set up hosting once**: in the fork's *Settings → Pages*, set the source to
   "GitHub Actions". Point Pages CMS at the `main` branch; content commits land
   there directly and only the "Publicar cambios" button (`.pages.yml` →
   `actions` → `deploy_page.yml`) deploys.
